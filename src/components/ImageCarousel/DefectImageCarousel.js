...
class DefectImageCarousel extends Component {
  ...
  render() {
    ...
    const prevButton =
      images.length > 1 ? (
        <button className={classNames(css.prev, css.shrinkNavigationButton)} onClick={this.prev} />
      ) : null;
    const nextButton =
      images.length > 1 ? (
        <button className={classNames(css.next, css.shrinkNavigationButton)} onClick={this.next} />
      ) : null;
    ...
    const imageClasses = classNames(css.defectImage, {
